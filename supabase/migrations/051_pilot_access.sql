...
-- Plano interno de cortesia. Nunca aparece no site comercial e nunca gera cobrança.
insert into public.nexus_plans (
  ...
  customer_type,
  organization_limit,
  employee_limit_scope
)
select
  product.id,
  'piloto',
  'Nexus SST Piloto',
  'Acesso interno de cortesia para validação técnica do Nexus SST.',
  0,
  'BRL',
  1,
  'active',
...
on conflict (product_id, code) do update set
  ...
  customer_type = 'single_company',
  organization_limit = 1,
  employee_limit_scope = 'organization',
  updated_at = now();
...
begin
  ...
  values (v_account_id, p_user_id, 'owner', true);

  insert into public.organization_memberships (organization_id, user_id, role, active)
  values (v_organization_id, p_user_id, 'org_admin'::public.app_role, true);
...
comment on function public.provision_nexus_pilot(uuid,uuid,text,text,text,text,integer)
  is 'Provisiona uma empresa única e isolada para piloto Nexus SST. Execução exclusiva do backend service_role após autorização nexus_admin.';

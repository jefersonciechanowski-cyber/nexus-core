(() => {
  ...
  function normalizeDate(value) {
    ...
    if (typeof value === 'number' && window.XLSX?.SSF?.parse_date_code) {
      ...
      return p ? isoDate({year:p.y, month:p.m, day:p.d}) : null;
    }
  ...
  function headerKey(value) {
    ...
    return ({
      'cpf':'cpf','cpf colaborador':'cpf','exame':'exam_name','nome do exame':'exam_name',
      'n coleta':'collection_number','numero coleta':'collection_number','numero da coleta':'collection_number','coleta':'collection_number',
      'data da coleta':'collected_at','data coleta':'collected_at','data':'collected_at',
      'resultado numerico':'numeric_value','valor':'numeric_value','resultado':'numeric_value',
      'resultado qualitativo':'qualitative_result','qualitativo':'qualitative_result'
    })[key] || '';
  }
  ...
  async function fetchAll(table, select, filters = []) {
    ...
    for (let from = 0;; from += 1000) {
      let query = getClient().from(table).select(select).range(from, from + 999);
  ...
  function normalizeExam(row) {
    ...
    return {
      ...
      evaluation: {mode:rule?.evaluation_mode || 'NONE', goodMin:rule?.good_min, goodMax:rule?.good_max, attentionMin:rule?.attention_min, attentionMax:rule?.attention_max}
    };
  }
  ...
  function classifyNumeric(exam, value) {
    ...
    if ([rule.goodMin, rule.goodMax, rule.attentionMin, rule.attentionMax].some(item => item == null)) return 'SEM PARÂMETRO';
  ...
  async function loadReferences() {
    ...
    employees.forEach(employee => {
      ...
      employeeByCpf.set(cpf, [...(employeeByCpf.get(cpf) || []), employee]);
    });
    const examByName = new Map();
    examsRaw.map(normalizeExam).forEach(exam => {
      const key = normalize(exam.name);
      examByName.set(key, [...(examByName.get(key) || []), exam]);
    });
    return {employeeByCpf, examByName, requirementSet:new Set(requirements.map(item => `${item.sector_id}|${item.exam_id}`))};
  }
  ...
  function validateRows(rawRows, refs) {
    ...
    return rawRows.map((raw, index) => {
      ...
      const examNameInput = String(data.exam_name ?? '').trim();
      ...
      let employee = null, exam = null, qualitativeResult = qualitativeRaw, previewStatus = '', needsRegistration = false;

      if (!cpf || !isValidCpf(cpf)) errors.push('CPF obrigatório e válido.');
      else {
        const matches = refs.employeeByCpf.get(cpf) || [];
        if (!matches.length) { needsRegistration = true; errors.push('CPF não cadastrado. Cadastre o colaborador no SST Controle antes de importar.'); }
      ...
      if (!examNameInput) errors.push('Exame é obrigatório.');
      else {
        const matches = refs.examByName.get(normalize(examNameInput)) || [];
        if (!matches.length) errors.push(`Exame “${examNameInput}” não cadastrado ou inativo.`);
        else if (matches.length > 1) errors.push(`Exame “${examNameInput}” duplicado no catálogo.`);
        else exam = matches[0];
      }

      if (!Number.isInteger(collectionNumber) || collectionNumber < 1) errors.push('Nº Coleta deve ser inteiro maior ou igual a 1.');
      if (data.collected_at === undefined || data.collected_at === null || String(data.collected_at).trim() === '') errors.push('Data da Coleta é obrigatória.');
      else if (!collectedAt) errors.push('Data da Coleta inválida. Use DD/MM/AAAA ou AAAA-MM-DD.');
      ...
      if (exam?.resultType === 'NUMERIC') {
        ...
        if (!qualitativeRaw) errors.push('Resultado Qualitativo é obrigatório para este exame.');
          ...
          else { qualitativeResult = String(options[0].label); previewStatus = String(options[0].status || 'SEM PARÂMETRO'); }
        }
      }
      ...
      return {
        rowNumber, cpf, employeeName:employee?.full_name || 'Não cadastrado', examName:exam?.name || examNameInput || '—',
        collectionNumber:Number.isInteger(collectionNumber) ? collectionNumber : '—', collectedAt:collectedAt || '—',
        result:exam?.resultType === 'QUALITATIVE' ? (qualitativeResult || '—') : (Number.isFinite(numeric) ? String(numeric) : '—'),
        previewStatus, needsRegistration, errors,
        payload:errors.length ? null : {cpf, exam_name:exam.name, collection_number:String(collectionNumber), collected_at:collectedAt, numeric_value:exam.resultType === 'NUMERIC' ? String(numeric) : null, qualitative_result:exam.resultType === 'QUALITATIVE' ? qualitativeResult : null}
      };
    });
  }
  ...
  async function downloadTemplate() {
    ...
    const instructions = [
      ...
      ['CPF é obrigatório e deve pertencer a colaborador já cadastrado na empresa selecionada.'],
      ['CPF e Data da Coleta já estão formatados como TEXTO no modelo para preservar zero inicial e o padrão brasileiro.'],
      ['Digite a data como DD/MM/AAAA. Para planilhas externas, AAAA-MM-DD também é aceito e evita ambiguidades de idioma/região do Excel.'],
      ['Se o CPF não existir, o Nexus mostrará a pendência para cadastrar o colaborador e bloqueará o lote.'],
      ['O exame deve existir no catálogo e estar vinculado ao setor atual do colaborador.'],
      ['Exame NUMÉRICO: preencha somente Resultado Numérico. Exame QUALITATIVO: somente Resultado Qualitativo.'],
      [`Máximo de ${MAX_ROWS} coletas por arquivo.`],
      ['Havendo qualquer erro, nenhuma coleta é importada.']
    ];
    ...
    prepareTextColumns(collectionSheet);
  ...
  function renderPreview() {
    ...
    if (pending) byId('nexusCollectionRevalidate').onclick = revalidate;
  ...
  async function revalidate() {
    try {
      setStatus('Revalidando CPFs, exames e vínculos…');
      parsedRows = validateRows(sourceRows, await loadReferences());
      const invalid = parsedRows.filter(row => row.errors.length).length;
      renderPreview();
      setStatus(invalid ? `Ainda existem ${invalid} linha(s) com erro ou cadastro pendente.` : `${parsedRows.length} coleta(s) validadas. Revise e confirme a importação.`, invalid ? 'bad' : 'good');
    } catch (error) {
      setStatus(error.message || 'Não foi possível revalidar o arquivo.','bad');
    }
  }
  ...
  async function parseFile(file) {
    try {
      ...
      sourceRows = XLSX.utils.sheet_to_json(sheet, {defval:'', raw:false, dateNF:'dd/mm/yyyy'}).filter(row => Object.values(row).some(value => String(value).trim() !== ''));
      if (!sourceRows.length) throw new Error('Nenhuma coleta foi encontrada no arquivo.');
      if (sourceRows.length > MAX_ROWS) throw new Error(`O arquivo possui ${sourceRows.length} linhas. O limite é ${MAX_ROWS} coletas por lote.`);
      parsedRows = validateRows(sourceRows, await loadReferences());
      const invalid = parsedRows.filter(row => row.errors.length).length;
      renderPreview();
      setStatus(invalid ? `O arquivo possui ${invalid} linha(s) com erro. Corrija o Excel ou cadastre os CPFs pendentes; nenhuma coleta será importada enquanto houver erro.` : `${parsedRows.length} coleta(s) validadas. Revise a prévia e confirme para gravar no Nexus.`, invalid ? 'bad' : 'good');
    } catch (error) {
  ...
  function buildUi() {
    ...
    byId('nexusCollectionTemplate').onclick = async () => {
      try { await downloadTemplate(); setStatus('Modelo Excel gerado. CPF e Data estão protegidos como texto; preencha a aba “Coletas”.','good'); }
